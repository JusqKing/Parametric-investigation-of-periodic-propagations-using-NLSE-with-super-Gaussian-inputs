...
distance = 4;   % normalized to dispersion length
...
for N=2:4
    ...
    % grid on
    % 
    % title_str = sprintf("cyfig1N=%.1f;m=%.1f",N,mshape);
    % 
    % set(gcf,"Position",[403.4000 133 367.2000 586.4000])
...
% 1：Intensity-z
figure
imagesc(z,tau,abs(uu_0).^2');
set(gca,'XDir','normal')
xlabel('Distance (z/L_D)'); ylabel('Time (t/T_0)');
ylim([-2 2])
colorbar
title('Intensity-z')
saveas(gcf, strcat(title_str,'_intensity.png'));
% close(gcf)
...
grid on;
title('Energy - z');
saveas(gcf, strcat(title_str,'_energy.png'));
% close(gcf)
...
% 在峰值处标注 x 坐标
for i = 1:length(valid_locs_peaks)
    text(valid_locs_peaks(i), valid_pks_peaks(i), ...
        sprintf('%.2f', valid_locs_peaks(i)), ...
...
title('FFT Spectrum')
grid on
saveas(gcf, strcat(title_str,'_fft.png'));
